...
const { FiSend, FiPaperclip, FiX, FiEdit2, FiTrash2, FiUser, FiClock, FiMoreVertical, FiCheck, FiBell, FiBellOff } = FiIcons;

const GlobalChat = ({ isOpen, onClose }) => {
  const { currentUser, currentCompany } = useAuth();
  const { getGlobalMessages, sendGlobalMessage, deleteGlobalMessage, editGlobalMessage, setCurrentUser } = useChat();
  ...
  }, [isOpen, getGlobalMessages, setCurrentUser, currentUser]);
  ...
  const handleEditMessage = (messageId, newText) => {
    editGlobalMessage(messageId, newText);
    setMessages(getGlobalMessages());
  ...
  const toggleNotifications = () => {
    ...
    // In a real app, this would store the preference and register/unregister for notifications
    localStorage.setItem('meister-global-chat-notifications', !notifications ? 'true' : 'false');
  };
          ...
          >
            ...
            <div className="bg-white rounded-t-lg shadow-sm border border-gray-200 p-4 flex items-center justify-between">
              <div className="flex items-center space-x-4">
                <div>
                  <h1 className="text-xl font-bold text-gray-900">Firmen-Chat</h1>
                  <p className="text-gray-600">{currentCompany?.name || 'Alle Mitarbeiter'}</p>
                </div>
              </div>
              <div className="flex items-center space-x-3">
                <button
                  ...
                  title={notifications ? "Benachrichtigungen deaktivieren" : "Benachrichtigungen aktivieren"}
                >
                  <SafeIcon icon={notifications ? FiBell : FiBellOff} className="w-5 h-5" />
                </button>
